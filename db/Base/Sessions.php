...
abstract class Sessions implements ActiveRecordInterface
{
    ...
    public function setSession($v)
    {
        ...
        if ($this->session !== $v) {
            $this->session = $v;
            $this->modifiedColumns[SessionsTableMap::COL_SESSION] = true;
        }
    ...
    public function setUser($v)
    {
        ...
        if ($this->user !== $v) {
            $this->user = $v;
            $this->modifiedColumns[SessionsTableMap::COL_USER] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : SessionsTableMap::translateFieldName('Session', TableMap::TYPE_PHPNAME, $indexType)];
            $this->session = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : SessionsTableMap::translateFieldName('User', TableMap::TYPE_PHPNAME, $indexType)];
            $this->user = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : SessionsTableMap::translateFieldName('Sessionid', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(SessionsTableMap::COL_SESSION)) {
            $modifiedColumns[':p' . $index++]  = 'session';
        }
        if ($this->isColumnModified(SessionsTableMap::COL_USER)) {
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(SessionsTableMap::COL_SESSION)) {
            $criteria->add(SessionsTableMap::COL_SESSION, $this->session);
        }
        if ($this->isColumnModified(SessionsTableMap::COL_USER)) {
            $criteria->add(SessionsTableMap::COL_USER, $this->user);
        }
        if ($this->isColumnModified(SessionsTableMap::COL_SESSIONID)) {
    ...
    /**
     * Generic method to set the primary key (session column).
     *
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($makeNew) {
            $copyObj->setNew(true);
            $copyObj->setSession(NULL); // this is a auto-increment column, so set to default value
        }
    }
