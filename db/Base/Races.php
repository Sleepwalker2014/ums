...
/**
 * Base class that represents a row from the 'races' table.
 *
 *
 *
* @package    propel.generator..Base
*/
abstract class Races implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\RacesTableMap';
    ...
    /**
     * Initializes internal state of Base\Races object.
     */
    ...
    /**
     * Compares this with another <code>Races</code> instance.  If
     * <code>obj</code> is an instance of <code>Races</code>, delegates to
     * <code>equals(Races)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|Races The current object, for fluid interface
     */
     ...
     * @return $this|\Races The current object (for fluent API support)
     */
    public function setRace($v)
    {
        ...
        if ($this->race !== $v) {
            $this->race = $v;
            $this->modifiedColumns[RacesTableMap::COL_RACE] = true;
        }
    ...
    public function setCode($v)
    {
        ...
        if ($this->code !== $v) {
            $this->code = $v;
            $this->modifiedColumns[RacesTableMap::COL_CODE] = true;
        }
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[RacesTableMap::COL_NAME] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : RacesTableMap::translateFieldName('Race', TableMap::TYPE_PHPNAME, $indexType)];
            $this->race = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : RacesTableMap::translateFieldName('Code', TableMap::TYPE_PHPNAME, $indexType)];
            $this->code = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : RacesTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = RacesTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Races'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildRacesQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
    ...
    public function delete(ConnectionInterface $con = null)
    {
        ...
        if ($con === null) {
            $con = Propel::getServiceContainer()->getWriteConnection(RacesTableMap::DATABASE_NAME);
        }

        $con->transaction(function () use ($con) {
            $deleteQuery = ChildRacesQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                RacesTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->animalssScheduledForDeletion !== null) {
                if (!$this->animalssScheduledForDeletion->isEmpty()) {
                    foreach ($this->animalssScheduledForDeletion as $animals) {
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[RacesTableMap::COL_RACE] = true;
        if (null !== $this->race) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . RacesTableMap::COL_RACE . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(RacesTableMap::COL_RACE)) {
            $modifiedColumns[':p' . $index++]  = 'race';
        }
        if ($this->isColumnModified(RacesTableMap::COL_CODE)) {
            $modifiedColumns[':p' . $index++]  = 'code';
        }
        if ($this->isColumnModified(RacesTableMap::COL_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'name';
        }

        $sql = sprintf(
            'INSERT INTO races (%s) VALUES (%s)',
    ...
    {
        $pos = RacesTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['Races'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Races'][$this->hashCode()] = true;
        $keys = RacesTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getRace(),
            $keys[1] => $this->getCode(),
            $keys[2] => $this->getName(),
    ...
    {
        $criteria = new Criteria(RacesTableMap::DATABASE_NAME);

        if ($this->isColumnModified(RacesTableMap::COL_RACE)) {
            $criteria->add(RacesTableMap::COL_RACE, $this->race);
        }
        if ($this->isColumnModified(RacesTableMap::COL_CODE)) {
            $criteria->add(RacesTableMap::COL_CODE, $this->code);
        }
        if ($this->isColumnModified(RacesTableMap::COL_NAME)) {
            $criteria->add(RacesTableMap::COL_NAME, $this->name);
        }
    ...
    {
        $criteria = ChildRacesQuery::create();
        $criteria->add(RacesTableMap::COL_RACE, $this->race);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Races (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($makeNew) {
            $copyObj->setNew(true);
            $copyObj->setRace(NULL); // this is a auto-increment column, so set to default value
        }
    }
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Races Clone of current object.
     * @throws PropelException
     */
     ...
     * If this ChildRaces is new, it will return
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildRaces The current object (for fluent API support)
     */
    ...
    public function removeAnimals(ChildAnimals $animals)
    {
        if ($this->getAnimalss()->contains($animals)) {
            ...
            $this->animalssScheduledForDeletion[]= $animals;
            $animals->setRaces(null);
        }
    ...
    {
        return (string) $this->exportTo(RacesTableMap::DEFAULT_STRING_FORMAT);
    }
